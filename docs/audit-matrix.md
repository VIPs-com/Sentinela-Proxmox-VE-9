...
| 9 | `unattended-upgrades`, `needrestart` | Debian [UnattendedUpgrades](https://wiki.debian.org/UnattendedUpgrades), [needrestart](https://github.com/liske/needrestart) | **OK** + **Ajuste** (seção 9.1b: não promover `restart=a` como “fix” para SSH; ler modos `l`/`i`/`a`) |
...
|------------------------------|---------------------|--------|
| deb822 `pve-no-subscription` com `Suites: trixie` | [Package Repositories](https://pve.proxmox.com/wiki/Package_Repositories) — exemplo idêntico (arquivo sugerido `proxmox.sources`) | **OK** |
...
| Reinício de VMs/CT após alternar backend firewall | Explícito na seção nftables da wiki | **OK** — considerar adicionar nota no guia |
...
| Arquivo whitelist `parsers/s02-enrich/whitelists.yaml` | Estrutura pode variar; pós-instalação: confirmar com `cscli parsers list` / árvore `/etc/crowdsec` | **Ajuste** (nota de validação no guia) |
| Interacção CrowdSec (nft) + `proxmox-firewall` | Vários consumidores nft no mesmo host; inspecionar `nft list ruleset` + docs bouncer + wiki Firewall | **Ajuste** (nota qualitativa no guia Fase 4) |
...
| Sugestão | Motivo |
|----------|--------|
| `sed` para `$nrconf{restart} = 'a'` como “solução” a desconexões SSH | Modo **`a`** = reinício **automático** de serviços; pode **aumentar** surpresas. Em `unattended-upgrades` não interativo, o modo interactivo pode fazer fallback a *list-only* — ver [needrestart.conf](https://github.com/liske/needrestart/blob/master/ex/needrestart.conf). |
...
| `tailscale status \| grep Userspace` como verificação canônica | Frágil entre versões; o guia usa **`ip addr show tailscale0`** (teste objetivo). |
| [ProxMenux](https://proxmenux.com/) como atalho de administração | Projeto de terceiros; o guia trata-o como **opcional** com aviso de *supply chain* (ver seção “Antes de Começar” e FAQ). |
...
| Tópico | Achado | Ação no guia |
|--------|--------|-------------|
| **CrowdSec versão** | Ainda v1.x (v1.7.x) — sem quebra de API v2 real; `install.crowdsec.net` é o script oficial correto para Debian Trixie (packagecloud.io desatualizado) | Sem alteração necessária; script já usa URL correta |
| **nftables `proxmox-firewall`** | Confirmado **tech preview** no PVE 9 (Roadmap Proxmox: melhorias em curso, não passou a estável) | Guia já documenta corretamente como tech preview |
| **Tailscale LXC** | Docs oficiais Tailscale recomendam editar `/etc/pve/lxc/100.conf` diretamente (`lxc.cgroup2.devices.allow: c 10:200 rwm` + `lxc.mount.entry`); guia usa `pct set --dev0` (método alternativo); ambos funcionam em PVE 8/9 com Tailscale 1.76+ | Guia já tem link para doc oficial Tailscale LXC unprivileged — link é o fallback canônico |
| **Docker Debian 13** | Repositório oficial Docker usa `trixie` no URL: `download.docker.com/linux/debian trixie stable`; Fase 8 usa `get.docker.com` (script convenience) que suporta Trixie | OK; `get.docker.com` funciona; nota de alternativa APT manual seria melhoria futura |

## Próximas reverificações recomendadas

- Reexecutar `apt-cache policy` / versões CrowdSec ao mudar de minor PVE ou Debian.
- Reler a seção **nftables** da [wiki Firewall](https://pve.proxmox.com/wiki/Firewall#nftables) antes de cada major upgrade do Proxmox.
- Confirmar status **proxmox-firewall** (tech preview → estável) a cada release major PVE.
- Verificar `cscli` subcommands se CrowdSec passar de v1.x para v2.x.
