...
## Escopo da versão 1.0 (canônica)

Esta é a **única versão publicada** neste repositório: não há v4/v5 anteriores no GitHub — tudo o que você precisa está neste arquivo e nos complementos em `docs/`.
...
5. 📝 **Documente cada mudança** no diário do laboratório `~/sentinela-lab/diario.md` (a pasta é criada na **Fase 1**; se ainda estiver na Fase 0, pode fazer uma vez `mkdir -p ~/sentinela-lab` e usar esse arquivo, ou anotar temporariamente em outro lugar até chegar lá).
...
Crie uma pasta "Sentinela Proxmox" e prepare entradas para:
...
```
Hostname (FQDN): sentinela.local       ← ou o nome que quiser; deve ter .local ou .domínio
...
- Guarde **imediatamente** no Bitwarden → pasta "Sentinela Proxmox" → entrada "Root Proxmox"
...
```bash
echo "## $(date +"%F %H:%M") - Fase -1 concluída" >> ~/sentinela-lab/diario.md
echo "- Proxmox VE instalado, acesso web e SSH OK" >> ~/sentinela-lab/diario.md
```
...
echo "## $(date +"%F %H:%M") - Fase 1 concluída" >> ~/sentinela-lab/diario.md
echo "- Criado usuário renato com sudo" >> ~/sentinela-lab/diario.md
```
...
> - `-f "$HOME/.ssh/chave_sentinela"` — onde salvar (evita sobrescrever sua chave padrão)
...
- `chave_sentinela` (privada — **NUNCA compartilhe, nunca faça upload**)
- `chave_sentinela.pub` (pública — pode ser distribuída)
...
```
# ===========================================
# Hardening SSH - Sentinela Proxmox
# ===========================================
...
```bash
ssh sentinela                      # Deve entrar direto
...
```bash
echo "## $(date +"%F %H:%M") - Fase 2 concluída" >> ~/sentinela-lab/diario.md
echo "- SSH só com chave Ed25519, drop-in em sshd_config.d/" >> ~/sentinela-lab/diario.md
```
...
> 4. **Na nova janela:** `ssh sentinela` (ou `ssh -i ~/.ssh/chave_sentinela renato@SEU_IP` se não usaste o §2.4) → deves ver `Verification code:` depois da autenticação por chave. Se **não** pedir código, o código falhar sempre, ou a ligação cair, **não** avances para o §3.5 nem removas o `nullok` — reverifica `/etc/pam.d/sshd`, o drop-in e `sudo sshd -T | grep -iE 'authenticationmethods|kbdinteractiveauthentication'`.

### ✅ Verifique

```bash
# Se configuraste o Host "sentinela" no §2.4:
ssh sentinela
# Senão: ssh -i ~/.ssh/chave_sentinela renato@192.168.1.100   (troca o IP)
```
...
```bash
echo "## $(date +"%F %H:%M") - Fase 3 concluída" >> ~/sentinela-lab/diario.md
echo "- 2FA TOTP ativo no SSH, nullok removido" >> ~/sentinela-lab/diario.md
```
...
```bash
echo "## $(date +"%F %H:%M") - Fase 4 concluída" >> ~/sentinela-lab/diario.md
echo "- CrowdSec + bouncer nftables ativo" >> ~/sentinela-lab/diario.md
```
...
3. **Port:** `22`
4. **Username:** `renato`
5. **Auth:** Key — importe a chave privada `chave_sentinela` (transfira via AirDrop, e-mail cifrado, ou Bitwarden Attachment)
...
```bash
echo "## $(date +"%F %H:%M") - Fase 5 concluída" >> ~/sentinela-lab/diario.md
echo "- CT 100 vpn-tailscale ativo com subnet routing" >> ~/sentinela-lab/diario.md
echo "- IP Tailscale (IPv4) do CT 100: $(sudo pct exec 100 -- tailscale ip -4)" >> ~/sentinela-lab/diario.md
```
...
```bash
echo "## $(date +"%F %H:%M") - Fase 6 concluída" >> ~/sentinela-lab/diario.md
echo "- renato@pam Administrator com 2FA TOTP" >> ~/sentinela-lab/diario.md
```
...
ssh sentinela                 # → deve funcionar (se configuraste Host no §2.4)
# Senão: ssh -i ~/.ssh/chave_sentinela renato@192.168.1.100
...
| Tailscale client | `ping sentinela` (100.x.x.x) | ✅ Responde (regra ICMP Tailscale) |
...
ping -c 3 sentinela        # deve responder (se hostname configurado)
...
```bash
echo "## $(date +"%F %H:%M") - Fase 7 concluída" >> ~/sentinela-lab/diario.md
echo "- Firewall nftables com DROP, 6 regras ACCEPT (SSH, GUI, ICMP para LAN e Tailscale)" >> ~/sentinela-lab/diario.md
echo "- Host invisível para internet, visível na LAN e Tailscale" >> ~/sentinela-lab/diario.md
echo "- Port forwarding removido do roteador" >> ~/sentinela-lab/diario.md
```
...
```bash
echo "## $(date +"%F %H:%M") - Fase 8 concluída" >> ~/sentinela-lab/diario.md
echo "- CT 200 lab-irmao-gpg com Docker + ShellHub agent" >> ~/sentinela-lab/diario.md
echo "- ShellHub Cloud pode ser usado como canal de emergência pessoal" >> ~/sentinela-lab/diario.md
```
...
```bash
echo "## $(date +"%F %H:%M") - Fase 9 concluída" >> ~/sentinela-lab/diario.md
echo "- unattended-upgrades + ferramentas instaladas" >> ~/sentinela-lab/diario.md
```

---

<span id="fase-10"></span>
...
## Acesso
- SSH: `ssh sentinela` (alias no ~/.ssh/config)
...
echo "## $(date +"%F %H:%M") - <Título da mudança>" >> ~/sentinela-lab/diario.md
echo "- <O que foi feito>" >> ~/sentinela-lab/diario.md
echo "- <Comando ou config relevante>" >> ~/sentinela-lab/diario.md
echo "" >> ~/sentinela-lab/diario.md
```
...
> **Bónus (repositório Git):** exemplos **systemd** (`.timer` + `.service`) para o mesmo horário sem editar `crontab`, o script **só-leitura** `sentinela-health-check.sh` (opção `--json` para agendadores), e **`make check`** / **`make check-json`** na raiz ([Makefile](../Makefile)). Índice: [scripts/README.md](../scripts/README.md).
...
# Ajuste o destino SSH: use o Host do §2.4 (ex.: sentinela) ou renato@IP_DO_PVE
rsync -avz --delete \
  sentinela:/root/backups/ \
  ~/sentinela-backups/
```

```bash
chmod +x ~/sync-sentinela-backups.sh
~/sync-sentinela-backups.sh
```
...
- TODOS no Bitwarden, pasta "Sentinela Proxmox"
...
git commit -m "Estado inicial da Sentinela Proxmox v1.0"
```
...
sudo bash /caminho/para/scripts/sentinela-health-check.sh --verbose
```

Se clonou o repositório no host (opção Git do §10.6), ainda mais simples:

```bash
cd ~/sentinela-lab   # ou onde tiver o repo clonado
...
```bash
echo "## $(date +"%F %H:%M") - Fase 10 concluída" >> ~/sentinela-lab/diario.md
echo "- README, diário, recuperação criados" >> ~/sentinela-lab/diario.md
echo "- Backup automático via cron 03:00" >> ~/sentinela-lab/diario.md
```

---

<span id="fase-10b"></span>
...
```bash
echo "## $(date +"%F %H:%M") - Fase 10b concluída" >> ~/sentinela-lab/diario.md
echo "- Backup vzdump configurado (agendado via painel)" >> ~/sentinela-lab/diario.md
echo "- Testar restauro de CT/VM antes de confiar como único backup" >> ~/sentinela-lab/diario.md
```

---

<span id="fase-vm-01"></span>
...
O host Proxmox (Sentinela) é o cofre — não se aprende Linux dentro do cofre. A VM de estudo é o laboratório onde você experimenta, erra, aprende e recomeça sem medo. Com ZFS e Cloud-Init, recriar do zero demora menos de 2 minutos.
...
> **Não tem chave SSH ainda?** A Fase 2 do guia ensina a criar `chave_sentinela`. Use a mesma chave para a VM de estudo — mais seguro e sem senha.
...
ssh -i ~/.ssh/chave_sentinela aluno@192.168.1.110
...
```bash
echo "## $(date +"%F %H:%M") - Fase VM-01 concluída" >> ~/sentinela-lab/diario.md
echo "- VM 110 (debian-estudo) criada com Cloud-Init" >> ~/sentinela-lab/diario.md
...
- [ ] `~/.ssh/config` no PC com alias `sentinela`
...
- [ ] **Verificação final:** `sudo bash scripts/sentinela-health-check.sh --verbose` (ou `make check`) — todos os itens a verde ✅ — [scripts/README.md](../scripts/README.md)
...
pvesh get /nodes/sentinela/tasks --limit 30 | grep -i error || echo "Sem erros ✅"
```
...
(crontab -l 2>/dev/null; echo "0 4 2 * * /sbin/zpool status rpool | grep -q 'errors: No known' || echo 'ZFS ALERT: pool rpool com erros!' | mail -s 'ZFS Alert - Sentinela' root") | crontab -
...
<span id="apendice-b"></span>

# 📊 Apêndice B — Comandos de Monitoramento Diário

Cole isso em um arquivo `~/sentinela-lab/comandos.md`:
...
| Tarefas PVE com erro | `pvesh get /nodes/sentinela/tasks --limit 20 \| grep -i error` | Silêncio esperado |
...
# Script fica em /opt/sentinela-monitor/ — detalhes completos no doc acima
```
...
Copie este template para o seu `~/sentinela-lab/diario.md` sempre que criar um novo CT ou VM. Um diário bem estruturado vale mais do que qualquer wiki.
...
> **Dica:** mantenha um arquivo por serviço em `~/sentinela-lab/servicos/CT-101-adguard.md`. Em 6 meses você vai agradecer por ter documentado o motivo de cada decisão.

---

<span id="apendice-d"></span>

# 🚀 Apêndice D — Plano de Expansão com LXCs e Labs de Estudo

Com a Sentinela operacional, o hardware N5095 (16 GB RAM, 4 cores) tem capacidade para muito mais. Aqui está o roteiro completo — utilitários da rede, laboratórios de estudo e infraestrutura avançada.
...
4. **Settings → Notifications → Add → Telegram** → insira o Bot Token e Chat ID (do doc [monitoramento-telegram.md](monitoramento-telegram.md))
...
services:
  ...
  grafana:
    ...
    environment:
      - GF_SECURITY_ADMIN_PASSWORD=sentinela123   # trocar após login!
...
1. Acesse `http://192.168.1.400:3000` → login `admin` / `sentinela123` → **troque a senha** → Bitwarden
...
```bash
mkdir -p ~/ansible-sentinela/{inventory,playbooks,roles}
cd ~/ansible-sentinela

# Arquivo de inventário — define os hosts que o Ansible gerencia:
cat > inventory/hosts.ini << 'EOF'
[pve]
sentinela ansible_host=100.X.X.X ansible_user=renato ansible_python_interpreter=/usr/bin/python3
...
# Esperado: sentinela | SUCCESS, adguard | SUCCESS, vaultwarden | SUCCESS
```
...
**Regra prática para o Sentinela:**
...
<span id="apendice-f"></span>

# 📚 Apêndice F — Glossário Expandido

Veja a seção **Glossário completo** no topo do documento (âncora interna `#glossario-completo`), ou o atalho [glossario.md](glossario.md).

---

<span id="apendice-g"></span>

# 🔐 Apêndice G — O Que Guardar no Bitwarden

Crie a pasta "Sentinela Proxmox" com:

| # | Item | Onde aparece no guia |
|---|------|----------------------|
| 1 | Senha root Proxmox | Instalação inicial |
| 2 | Senha usuário `renato` | Fase 1 |
| 3 | Passphrase chave SSH `chave_sentinela` | Fase 2 |
| 4 | Conteúdo da chave privada `chave_sentinela` | Fase 2 (backup) |
...
<span id="apendice-h"></span>

# 🛟 Apêndice H — Plano de Recuperação de Desastre

Já está documentado em `~/sentinela-lab/recuperacao.md` (criado na Fase 10).
...
# Ver VMs de um nó:
pvesh get /nodes/sentinela/qemu

# Ver status de uma VM:
pvesh get /nodes/sentinela/qemu/101/status/current

# Iniciar/parar VM via API:
pvesh create /nodes/sentinela/qemu/101/status/start
pvesh create /nodes/sentinela/qemu/101/status/stop

# Ver storage disponível:
pvesh get /nodes/sentinela/storage
...
pvesh get /nodes/sentinela/tasks --limit 20
...
pvesh get /nodes/sentinela/tasks/<UPID>/log
# Exemplo de UPID: UPID:sentinela:001A2B3C:00001234:6640A1B2:vzdump:110:root@pam:

# Filtrar só tarefas com erro:
pvesh get /nodes/sentinela/tasks --limit 50 | grep -i error
...
echo "=== Tarefas com erro ===" && pvesh get /nodes/sentinela/tasks --limit 30 | grep -i error || echo "Nenhum"
...
> **Dica:** se um backup noturno falhou mas você só viu no dia seguinte, `pvesh get /nodes/sentinela/tasks --limit 50 | grep -i error` mostra o UPID exato. Use o UPID para ler o log completo e saber exatamente o que falhou.
...
<span id="apendice-k"></span>

# 🛡️ Apêndice K — Postura de Segurança (o que você ganhou)

Após concluir as fases Sentinela, o seu Mini PC tem uma **base de segurança sólida para homelab**. Aqui está o que está protegido — e o que não está.
...
make check   # ou: sudo bash scripts/sentinela-health-check.sh --verbose
# Todos os itens ✅ = postura mantida
```

---

<span id="apendice-l"></span>

# 🎓 Apêndice L — Dicas Finais e Próximos Estudos

Você concluiu a Sentinela. O que vem a seguir?
...
echo "$(date +"%F") — aprendi: [o que foi]" >> ~/sentinela-lab/diario.md
...
alias health='sudo bash /root/scripts/sentinela-health-check.sh --verbose 2>/dev/null || make check'
```
...
> ⚠️ **Guarde este endereço no Bitwarden** — pasta "Sentinela Proxmox" → "Endereço .onion SSH". Sem ele não consegue aceder via Tor.
...
# Conectar ao .onion:
torsocks ssh -i ~/.ssh/chave_sentinela renato@abc123xyz456defg.onion
...
ssh -i /caminho/para/chave_sentinela \
    -o ProxyCommand="nc -X 5 -x 127.0.0.1:9050 %h %p" \
    renato@abc123xyz456defg.onion

# Ou com torsocks (disponível no Tails):
torsocks ssh -i /caminho/para/chave_sentinela renato@abc123xyz456defg.onion
```
...
```bash
echo "## $(date +"%F %H:%M") - Apêndice N concluído" >> ~/sentinela-lab/diario.md
echo "- CT 103 (tor-hidden) ativo com Hidden Service" >> ~/sentinela-lab/diario.md
echo "- Endereço .onion guardado no Bitwarden" >> ~/sentinela-lab/diario.md
echo "- Chaves Tor com backup cifrado GPG" >> ~/sentinela-lab/diario.md
```

---

<span id="apendice-i"></span>
