...
Este documento **não** obriga a compras nem a fases rígidas: descreve a **evolução pensada** do laboratório, do projeto anterior *Linux Foundation Lab* para o caminho atual **Sentinela Proxmox**.

## Fase atual — Mini PC (Proxmox no N5095)
...
**Papel:** hypervisor Proxmox + rede segura (guia Sentinela) + VMs/CTs de estudo (firewall, DMZ, GPG, etc.).

---

## Fase intermediária (quando fizer sentido) — estação mais forte
...
Não é obrigatório: você pode ficar só no mini PC até dominar o percurso do guia.
...
Isso é **horizonte** de aprendizagem (Proxmox cluster, storage avançado), não requisito do guia v1.0.
...
- **Sem port forwarding** na Internet para expor o lab (alinhado ao Sentinela: Tailscale / acesso controlado).  
